...
def _scan_extrema(arr, kt, ky, A, Bm, N, K, tid, nthreads, want_max, collect):
    ...
    # Process the array in contiguous tiles to preserve strict ascending i
    for base in range(1, N - 1, nthreads):
        ...
        # Hillis-Steele inclusive prefix sum on the 128-element tile (A/B ping-pong)
        for k in range(7):  # 7 steps for 128 threads
            step = 1 << k
            if k & 1:
        ...
        # After 7 steps (k=0 to 6), the final inclusive sum is in Bm.
        tile_total = Bm[nthreads - 1]
        
        # Compact: if this thread found a hit, write it to global memory
        if collect and hit == 1:
            # Exclusive offset inside the tile is inclusive_sum - 1
            tile_off = Bm[tid] - 1
            pos = total + tile_off
            if pos < K:
                kt[pos] = np.int32(i)
...
def gpu_emd_single(x, t=None, tau=0.25, max_sifts=50, K=None, nthreads=128,
    ...
    kern[1, nthreads](S_arr, resid, h, out, eu, W, W_int, Wwin, nsift, nmode,
                      tau, max_sifts, 4)
