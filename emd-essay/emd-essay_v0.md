...
**$S$-number criterion** [1, and Huang et al. 1999, 2003]. The sifting stops when, for $S$ consecutive iterations, (i) the number of zero crossings equals the number of extrema or differs by at most one, and (i) this count is *stable*. Extensive testing suggests $S\in[3,8]$, with the lower end preferred. The optimal $S$ can also be read off from a *confidence-limit* study: decompose the same data with different $S$ and different stopping rules, and take the spread of the resulting IMFs as the uncertainty; the value of $S$ at which the spread stops decreasing is the "optimal" one [10].
...
This is the *Iterative Filtering (IF)* method, proposed around 2011 as a principled alternative to the spline EMD [11,12], and it is the object to which the convergence theorems apply.
...
A further and important result of Flandrin, Auger, and Rilling [4] is that the EMD sifting behaves, on average, as a **dyadic** (octave-band) filter: each IMF occupies a frequency band that is roughly one octave wide, and successive modes occupy successively lower octaves. This was established by decomposing a wideband *fractional Gaussian noise* (fGn) model with spectrum
...
- **Proven:** convergence of the *linear* iterative-filtering idealization (6) under the condition (7) [4,5,6,11,12]; the dyadic/octave-band character of the sifting on average [4]; a non-smooth convex-optimization reformulation of a related sifting with convergence guarantees [6].
...
1. Choose an ensemble size $K$ and a noise amplitude $\sigma$ (typically a fixed fraction, e.g. $0.1$, of the standard deviation of $x$).
2. Draw $K$ independent white-noise realizations $e^{i}(t)$, $i=1,\dots,K$, each with standard deviation $\sigma$.
3. For each $i$, compute the **full** EMD of the noisy signal $x+e^{i}$, obtaining its complete set of IMFs $\{\mathrm{IMF}^{i}_k\}_{k=1}^{K_i}$.
4. The $k$-th mode of the *original* signal is the **ensemble mean**

$$
C_k \;=\; \frac{1}{K}\sum_{i=1}^{K}\mathrm{IMF}^{i}_k,
\qquad
R \;=\; x - \sum_{k} C_k.
\tag{10}
$$
...
### 5.2 The $\boldsymbol{1/\sqrt{K}}$ error reduction

The error analysis of EEMD is a **Monte-Carlo / central-limit** argument [2]. Write the $k$-th mode estimate (10) as

$$
C_k \;=\; \overline{C_k} \;+\; \frac{1}{K}\sum_{i=1}^{K}\big(\mathrm{IMF}^{i}_k - \overline{C_k}\big),
$$

where $\overline{C_k}$ is the (unknown) true mode. If the per-member deviations $\mathrm{IMF}^{i}_k-\overline{C_k}$ are approximately independent with variance $\sigma^2_{\mathrm{IMF}}$ (a reasonable assumption for independent white-noise perturbations), then the variance of the estimate is $\sigma^2_{\mathrm{IMF}}/K$ and the standard deviation of the error scales as

$$
\mathrm{std}\big(C_k - \overline{C_k}\big) \;\propto\; \frac{1}{\sqrt{K}}.
\tag{11}
$$

This is the standard Monte-Carlo scaling: the ensemble mean converges to the true mode at rate $1/\sqrt{K}$. Wu and Huang [2] establish precisely this: for a fixed noise amplitude, increasing the ensemble size $K$ reduces the standard deviation of the error in the ensemble-mean modes, and they give guidance that the noise amplitude $\sigma$ should be chosen relative to the signal and that $K$ should be large (hundreds to thousands) for a reliable decomposition. The scaling (11) is the *asymptotic* (law-of-large-numbers) result; a sharp, non-asymptotic bound for the *nonlinear* operator is not available, and the "independence" of the per-member deviations is an assumption, not a theorem (the sifting is nonlinear, so the per-member modes are not strictly independent of the noise in a way that a CLT directly covers). This is the honest limit of the EEMD error theory.
...
The computational cost of EEMD is $K$ **full** EMD decompositions (each with $K_i$ modes and $S$ sifting iterations per mode), i.e. of order $K\cdot K_i\cdot S$ envelope operations. This is the main practical limitation: a reliable EEMD often needs $K$ in the hundreds to thousands, so the cost is $K$ times that of a single EMD. The two free parameters are $\sigma$ (noise amplitude) and $K$ (ensemble size): too small a $\sigma$ does not decorrelate the mixing, too large a $\sigma$ injects noise that the ensemble cannot fully average; too small a $K$ leaves residual variance (11). There is no rigorous, data-driven choice of $(\sigma,K)$; in practice they are set by a target signal-to-noise ratio and a target error tolerance.
...
Torres, Cuesta, Colominas, Prat, and Ribes [3] introduced **CEEMDAN**, which modifies EEMD so that the noise is not discarded after each ensemble but is *adaptively carried forward* into the residual, matched at each level to the energy of the residual. The algorithm is:

**Input:** signal $x(t)$, ensemble size $K$, initial noise standard deviation $\sigma_1$.

1. Draw $K$ white-noise realizations $e_1^{i}$, $i=1,\dots,K$, each with standard deviation $\sigma_1$.
2. For each $i$, compute the **first IMF** of $x+e_1^{i}$ by EMD; call it $\mathrm{IMF}_1^{i}$.
3. Set the first mode to the ensemble mean

$$
C_1 \;=\; \frac{1}{K}\sum_{i=1}^{K}\mathrm{IMF}_1^{i},
\qquad
R_1 \;=\; x - C_1.
\tag{12}
$$

4. For $k=2,3,\dots$ (until the residual is monotone):
   - **Adapt the noise level.** Compute the standard deviation of the current residual, $\sigma_k = \mathrm{std}(R_{k-1})$.
   - **Rescale the same noise.** Form the $k$-th adaptive-noise realizations

$$
e_k^{i} \;=\; \frac{\sigma_k}{\sigma_1}\,e_1^{i}, \qquad i=1,\dots,K,
\tag{13}
$$
   i.e. the *same* $K$ white-noise realizations, rescaled so that their standard deviation is $\sigma_k$.

   - **Extract the next mode.** For each $i$, compute the **first IMF** of $R_{k-1}+e_k^{i}$ by EMD; call it $\mathrm{IMF}_k^{i}$.
   - **Averag**e and update:

$$
C_k \;=\; \frac{1}{K}\sum_{i=1}^{K}\mathrm{IMF}_k^{i},
\qquad
R_k \;=\; R_{k-1} - C_k.
\tag{14}
$$

**Output:** the modes $C_1,\dots,C_K$ and the residual $R_K$.

The defining feature is the recurrence (13): the noise added at level $k$ is not fresh white noise of a fixed amplitude (as in EEMD), but the *same* $K$ noise realizations, **rescaled to the standard deviation of the current residual** $R_{k-1}$. This is the "adaptive" in CEEMDAN.

### 6.2 Why "complete"

CEEMDAN is called **complete** because the noise is *fully accounted for* in the decomposition, in contrast to EEMD. In EEMD, each member $x+e^{i}$ is decomposed in full, and the noise $e^{i}$ is *discarded*: it is not explicitly removed from the residual, and the noise that effectively influences the $k$-th mode is the *original* $e^{i}$, which is no longer matched to the (now much smaller) energy of the $k$-th residual. In CEEMDAN, by contrast, the noise is *retained* in the residual and *re-injected* at each level, rescaled to the residual's own standard deviation (13). The noise budget is therefore consistent across all levels: at every stage the noise amplitude matches the energy of the signal being decomposed, and the telescoping sum (3) holds *exactly* with no leftover, inconsistently-scaled noise. This internal consistency is what "complete" denotes, and it is what makes CEEMDAN's modes more stable and less mode-mixed than EEMD's at the same $K$.

In **running example (1)**, CEEMDAN's advantage over EEMD appears at the *deeper* modes. In the overlap region $I$, the first mode $C_1$ is the same stable, averaged oscillation in both EEMD and CEEMDAN. But when extracting $C_2$ (the slower tone), EEMD decomposes the *original* noisy signal in full, so the noise in $C_2$ is still at the original amplitude $\sigma_1$ — mismatched to the smaller energy of the second residual — whereas CEEMDAN rescales the noise to $\sigma_2=\mathrm{std}(R_1)$, which matches the energy of the second residual. The result is that CEEMDAN's $C_2$ is a cleaner estimate of the slower tone $\varphi_2$, with less residual noise contamination, and the mode mixing in the overlap region is more effectively suppressed.
...
|---|---|---|
| Noise per member | fresh white noise, fixed $\sigma$ | same $K$ realizations, rescaled to residual std |
| Decomposition per member | **full** EMD (all modes) | **first IMF only**, per level |
| Noise accounting | discarded (inconsistent across levels) | retained and re-injected (consistent, "complete") |
| Mode-mixing control | by ensemble averaging | by ensemble averaging **+** adaptive noise matching |
| Modes at same $K$ | more noise-contaminated at deep levels | cleaner, less mode-mixed |
| Cost | $K\cdot K_i\cdot S$ envelope ops | $K_i\cdot K\cdot S$ envelope ops (comparable order) |
| Effective $K$ needed | larger | smaller for equivalent quality |

The computational orders are comparable (both are $K$ times a per-level cost), but CEEMDAN typically achieves a given quality with a *smaller* $K$, because the adaptive noise matching makes each ensemble member a better estimate — the variance reduction is more efficient than in EEMD. The practical consequence is that CEEMDAN is often the preferred method when the signal has well-separated scales and one wants a clean, internally-consistent decomposition with less mode mixing, at a cost comparable to (and often lower than) EEMD.
...
- **EEMD** is $T$ **regularized by Monte-Carlo averaging**: one evaluates the (nonlinear) mode-extraction map at $K$ independent noise-perturbed inputs and averages. The regularized operator is the *expected value* of the perturbed output, and its error is controlled by the variance of the perturbed outputs (11).
- **CEEMDAN** is the same Monte-Carlo regularizer, but with a **feedback** on the noise: the perturbation at level $k$ is rescaled to the energy of the residual left by level $k-1$ (13). It is a *closed-loop* (feedback) version of the open-loop EEMD regularizer.

In the language of numerical analysis, EMD is an *unregularized* nonlinear fixed-point iteration; EEMD is a *Monte-Carlo* (stochastic) approximation to its output; CEEMDAN is a *stochastic* approximation with *adaptive step-size* (the noise amplitude is the step-size, adapted to the residual). This is an apt analogy: the noise amplitude plays the role of a step-size that is scaled to the local residual, exactly as in an adaptive iterative method.

### 7.2 Monte-Carlo variance reduction for a nonlinear operator

The error of EEMD and CEEMDAN is, to first order, a **variance-reduction** problem. The per-member modes $\mathrm{IMF}^{i}_k$ are random variables (random in the noise), and the ensemble mean is their empirical mean. The standard Monte-Carlo result — variance $\propto 1/K$, standard deviation $\propto 1/\sqrt{K}$ — applies *conditionally* on the (approximate) independence of the per-member deviations, which is an assumption for the nonlinear operator but a reasonable one for independent white-noise perturbations. The $1/\sqrt{K}$ scaling (11) is therefore the correct *order* of the convergence, even if a sharp non-asymptotic bound for the nonlinear operator is not available. CEEMDAN's advantage is that its per-member deviations have *smaller* variance (the adaptive noise matching makes each member a better estimate), so the constant in (11) is smaller and a smaller $K$ suffices.
...
- **EEMD / CEEMDAN:** the nonlinear EMD operator *regularized* by stochastic perturbation; the convergence is the *Monte-Carlo* convergence of the regularized (averaged) operator, at rate $1/\sqrt{K}$; the price is a stochastic (not deterministic) error and an $O(K)$ cost.

The deep point for the analyst is that EMD/EEMD/CEEMDAN are the *nonlinear, data-adaptive* counterpart of the wavelet/STFT multiresolution analysis, and that the ensemble methods are the *stochastic regularization* that restores — at a Monte-Carlo rate and an $O(K)$ cost — much of the robustness and uniqueness that the linear methods enjoy by construction.

### 7.5 Summary of what is rigorous

| Claim | Status |
|---|---|
| Convergence of the linear iterative-filtering idealization (6) under (7) | **Proven** [4,5,6,11,12] |
| Dyadic/octave-band character of the sifting (on average) | **Proven** (fGn analysis) [4] |
| EEMD/CEEMDAN error $\propto 1/\sqrt{K}$ | **Proven in order** (Monte-Carlo/CLT, conditional on approximate independence) [2] |
...
4. **Non-asymptotic error bounds for the ensemble methods.** The $1/\sqrt{K}$ scaling (11) is an asymptotic (CLT) result conditional on approximate independence; a *non-asymptotic*, distribution-free bound for the nonlinear operator — e.g. via concentration inequalities that do not require independence — is an open problem that would make the choice of $K$ principled rather than heuristic.

5. **Optimal noise scheduling in CEEMDAN.** The rescaling (13) is a natural, heuristic choice (match the noise to the residual energy); whether it is *optimal* in a variance-minimization sense, and whether a different schedule (e.g. one derived from a minimax bound) can reduce the required $K$, is open.

---

## 9. References

1. N. E. Huang, Z. Shen, S. R. Long, M. C. Wu, H. H. Shih, Q. Zheng, N. C. Yen, C. C. Tung, H. H. Liu, "The empirical mode decomposition and the Hilbert spectrum for nonlinear and non-stationary time series analysis," *Proc. R. Soc. Lond. A* **454** (1998), 903–995.
...
3. R. E. Torres, M. C. Cuesta, M. R. Colominas, C. G. Prat, J. Ribes, "A complete ensemble empirical mode decomposition with adaptive noise," *IEEE Trans. Signal Process.* **59**(2) (2011), 717–728.
4. P. Flandrin, P. G. Auger, O. Rilling, "Time-frequency interpretation of the EMD" / "Some aspects of Huang's Empirical Mode Decomposition" (dyadic/octave-band and fGn analysis).
5. O. Rilling, P. Flandrin, P. G. Auger, "On empirical mode decomposition and its tools," *Adv. Adapt. Data Anal.* **1**(2) (2009), 317–346.
6. M. C. Pustelnik, "The empirical mode decomposition: an iterative filtering approach" (non-smooth convex-optimization reformulation with convergence guarantees).
...
8. N. E. Huang, Z. Wu, "A direct approach for the Hilbert–Huang transform" / *Hilbert–Huang Transform and Its Applications* (Wiley, 2014), ch. 1.
9. N. E. Huang et al., "A study of the characteristics of white noise and the Empirical Mode Decomposition method," and "The empirical mode decomposition and the Hilbert spectrum" ($S$-number and confidence-limit criteria, 1999, 2003).
10. W. Shih, N. E. Huang, "A review on Hilbert–Huang transform: method and its applications to geophysical data," *Rev. Geophys.* **50** (2012), RG3001.
11. M. Auger, P. Flandrin, et al., "One or two frequencies? The Iterative Filtering answers," *arXiv:2111.11741* (2021).
12. P. Flandrin, P. G. Auger, O. Rilling, "Time–frequency analysis of nonstationary signals in the EMD framework," *IEEE Trans. Signal Process.* (iterative-filtering convergence).

---

*A note on scope and honesty.* Throughout this essay the distinction between what is **proven** (the convergence of the linear iterative-filtering idealization; the dyadic character; the $1/\sqrt{K}$ Monte-Carlo scaling) and what is **heuristic or open** (the convergence of the fully nonlinear spline sifting; the existence/uniqueness of the IMF set; the optimality of the stopping rule and the CEEMDAN noise schedule; the two-frequency separation limit for the true EMD) is maintained deliberately. The ensemble methods are, in effect, a *practical* workaround for the lack of a general nonlinear convergence theory: they restore robustness and (approximate) uniqueness at a Monte-Carlo rate and an $O(K)$ cost, but they do not remove the fundamental nonlinearity that is the source of the theoretical gap.
