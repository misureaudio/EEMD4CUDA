...
| Convergence | no general theorem | **guaranteed** (linear operator) |
| Adaptivity | fully data-driven | fixed basis (not adaptive to the signal) |
| Mode mixing | yes | no (linear reassignment) |

Synchrosqueezing is the *linear* counterpart of EMD: it achieves a similar multiresolution (mode-by-mode) decomposition but with the full convergence and stability guarantees of a linear operator, at the price of losing the data-adaptivity. The recent two-frequency analysis [11] compares EMD, iterative filtering, and synchrosqueezing on the question of how well two close-by frequencies can be separated, and finds that the methods have complementary strengths: EMD/IF are more adaptive, synchrosqueezing is more stable.

### 9.3 The trade-off

The EMD family and its competitors embody a fundamental trade-off in multiresolution analysis: **adaptivity vs. rigor**. EMD and its ensemble refinements are fully data-adaptive (no fixed basis, data-driven mode count) but lack a general convergence theorem; VMD and synchrosqueezing are more rigorous (variational or linear) but sacrifice some adaptivity (VMD requires a fixed $K$; synchrosqueezing uses a fixed basis). The ensemble methods (EEMD, CEEMD, CEEMDAN, ICEEMDAN) are the EMD family's answer to the rigor gap: they restore robustness and (approximate) uniqueness via stochastic regularization, at a Monte-Carlo rate and an $O(N)$ cost, but they do not remove the fundamental nonlinearity that is the source of the theoretical gap.
